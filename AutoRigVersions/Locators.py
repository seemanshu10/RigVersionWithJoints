...
def globalNames(spineValue):
	global spineCount
	
def ReturnSpineAmount():
    return spineCount

def generateLocators(spineValue):
    global spineCount

    spineCount =spineValue

    # check the LOC_Master grp exist dont do anything
    # otherwise create the empty grp
    ...
    if rig_Type == 1:
        if side == 1:  # for Left_arm
            if cmds.objExists("L_Arm_GRP"):
                ...
                cmds.move(4 * side, 1 + (8 *spineCount), 0, L_arm)
                # upper Arm joints
                for i in range(0, 3):
            ...
            if cmds.objExists('R_Arm_GRP'):
                ...
                cmds.move(4 * side, 1 + (8 * spineCount), 0, R_arm)
                # upper Arm joints
                for i in range(0, 3):
        ...
        if side == 1:  # for Left_Leg
            if cmds.objExists("L_Leg_Front_GRP"):
                ...
                cmds.parent(L_leg_front, 'Loc_SPINE_' + str(spineCount - 1))
                cmds.move(2 * side, 1 + (2 * spineCount), -7, L_leg_front, r=1)
            ...
            if cmds.objExists('R_Leg_Front_GRP'):
                ...
                cmds.parent(R_leg_front, 'Loc_SPINE_' + str(spineCount - 1))
                cmds.move(2 * side, 1 + (2 * spineCount), -7, R_leg_front, r=1)
...
def createFingers(side,handPos,count):
    for x in range(0,4):
        if side ==1:
            ...
            cmds.move(handPos[0]+(0.5+(0.5*x))*side,handPos[1]-(0.5+(0.5*x)),handPos[2]+ -(1*count),finger)
        else:
    ...
    if rig_Type == 1:
        if side == 1:  # for Left_Leg
            if cmds.objExists("L_Leg_GRP"):
                ...
                cmds.parent(L_leg, 'Loc_SPINE_' + str(spineCount- 1))
                cmds.move(4 * side, 1 + (4 * spineCount), 0, L_leg)
                # upper Arm joints
                for i in range(0, 5):
            ...
            if cmds.objExists('R_Leg_GRP'):
                ...
                cmds.parent(R_leg, 'Loc_SPINE_' + str(spineCount - 1))
                cmds.move(4 * side, 1 + (4 * spineCount), 0, R_leg)
                # upper Arm joints
                for i in range(0, 5):
        ...
        if side == 1:  # for Left_Leg_Back
            if cmds.objExists("L_Leg_Back_GRP"):
                ...
                cmds.parent(L_leg_back, 'Loc_SPINE_' + str(spineCount- 1))
                cmds.move(2 * side, 1 + (2 * spineCount), 8, L_leg_back, r=1)
            ...
            if cmds.objExists('R_Leg_Back_GRP'):
                ...
                cmds.parent(R_leg_back, 'Loc_SPINE_' + str(spineCount - 1))
                cmds.move(2 * side, 1 + (2 * spineCount), 8, R_leg_back, r=1)
...
def createHead():
    for i in range(0, 3):#cmds.intField(3, q=True, value=True)
        ...
        if i == 0:
            cmds.parent(neck, 'Loc_SPINE_' + str(spineCount - 1))
        else:
            cmds.parent(neck, 'Loc_Neck_' + str(i - 1))

    temp =cmds.parentConstraint('Loc_SPINE_' + str(spineCount - 1),'Loc_Neck_0',  mo=False)
    cmds.delete(temp)
